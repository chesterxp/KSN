...
let FootballSchool = {
    ...
    isMobile: false,
    init: function() {
        let me = FootballSchool;
        let startPackBackground = document.querySelectorAll('.start-pack-background');
        let hamburger = document.querySelector('.hamburger-menu');
        let buttonStartPackActivities = document.querySelector('.start-pack-button');
        let buttonsStartPackListPrices = document.querySelectorAll('.price-list__description-down');
        me.slider = document.querySelector('.slider');

        // me.boxSlider = document.querySelector('.box_slider');
        if (me.slider) {

            me.sliderTitle = me.slider.querySelector('.slider__button-main-title');
            me.sliderFindOut = me.slider.querySelector('.slider__find-out');
            me.sliderPhoto = me.slider.querySelector('.slider-photo');
            me.sliderGraphic = me.slider.querySelector('.slider__grphic');
        }

        let animateElements = document.querySelectorAll('.animate-element');

        document.addEventListener("DOMContentLoaded", me.hideLoader);

        me.isMobile = me.isMobileDevice();

        if (!!hamburger) {
            me.hamburger(hamburger);
        }

        if (buttonStartPackActivities) {
            me.showStartPackActivities(buttonStartPackActivities, startPackBackground[0]);
            me.hideStartPack();
        }

        if (buttonsStartPackListPrices.length > 0) {
            me.addEventToShowStartPackPriceList(buttonsStartPackListPrices);
            me.hideStartPack();
        }
    ...
    showStartPackActivities: function(button, startPackBackground) {
        button.addEventListener('click', () => {
            startPackBackground.classList.toggle('show-start-pack');
        });
    },
    addEventToShowStartPackPriceList: function(buttons) {
        buttons.forEach((btn) => {
            btn.addEventListener('click', FootballSchool.showStartPackPriceList);
        })
    },
    showStartPackPriceList: function() {
        let typeOfpack = this.dataset.pack;
        let target = '';

        if (typeOfpack == 'football') {
            target = '.pack1';
        } else {
            target = '.pack2';
        }
        document.querySelector(target).classList.toggle('show-start-pack');
    },
    hideStartPack: function() {
        let closeButtons = document.querySelectorAll('.start-pack__close');
        closeButtons.forEach((btn) => {
            btn.addEventListener('click', (e) => {
                let target = e.target;
                let close = target.className;
                let parentBox;

                if (close == 'start-pack__close') {
                    parentBox = target.parentElement.parentElement;
                } else {
                    parentBox = target.parentElement.parentElement.parentElement;
                }
                parentBox.classList.toggle('show-start-pack');
            })
        })
    },
    sliderTurnOn: function() {
        let me = FootballSchool;
        setInterval(me.changeFoto, 5000);
    },
    changeFoto: function() {
        ...
        if (me.numberOfPhoto == 1 || me.numberOfPhoto == 4) {
            me.numberOfPhoto = 1;
            me.slider.classList.remove('gimfun');
            me.slider.classList.add('football');
            text = 'Akademia piłkarska';
            href = 'activities_football.html';
            graphicHref = 'activities_football.html#graphic';

        } else if (me.numberOfPhoto == 2) {
            text = 'Cheerleaders';
            href = 'activities_cheerleading.html';
            me.slider.classList.remove('football');
            me.slider.classList.add('cheerleder');
            graphicHref = 'activities_cheerleading.html#graphic';
        } else if (me.numberOfPhoto == 3) {
            me.slider.classList.remove('cheerleder');
            me.slider.classList.add('gimfun');
            text = 'GimFun';
            href = 'activities_acrobatics.html';
            graphicHref = 'activities_acrobatics.html#graphic';
        }

        me.sliderTitle.innerText = text;
        me.sliderFindOut.href = href;
        me.sliderGraphic.href = graphicHref;
        if (me.isMobile) {
            hrefOfPicture = `url(./images/mslider${me.numberOfPhoto}.jpg)`;
        } else {
            hrefOfPicture = `url(./images/slider${me.numberOfPhoto}.jpg)`;
        }
        me.sliderPhoto.style.backgroundImage = hrefOfPicture;
        me.numberOfPhoto++;
    },
    myObserver: function(targets, viewPortIn, viewPortOut, rootMargin = '5px') {
